...
plot_lev <- function(
  ...
  if (one_lev_num == length(Leverage)) {
    #Check all lev = 1
    const_lev <- TRUE
  } else {
    range_lev <- range(subset(Leverage, Leverage < cutoff), na.rm = TRUE)
    const_lev <- all(range_lev == 0) ||
      diff(range_lev) < 1e-10 * mean(subset(Leverage, Leverage < cutoff), na.rm = TRUE)
  }
  ...
  if (const_lev) {
    ...
    if (one_lev) {
      warning(
        "Observations with a leverage value of 1 are not included
    ...
    model_values <- 
      data.frame(
        Leverage = round(hatvalues(model), 5),
    ...
    hat_seq <- seq.int(
      min(range_lev[1], range_lev[2] / 100),
      0.999,
      length.out = max(100, model$df.residual)
    ) # Add more resolution for large n
    
    # Create the limits for the plot
    xlimits <- c(0, max(
      subset(model_values$Leverage, model_values$Leverage < cutoff),
      na.rm = TRUE
    ))
    ylimits <- extendrange(range(
      subset(model_values$Std_Res, model_values$Leverage < cutoff),
      na.rm = TRUE
    ), f = 0.1)
    ...
    ylable_pos_0.5 <- 
      1.05 * sqrt(0.5 * p * (1 - max(model_values$Leverage, na.rm = TRUE)) / 
                    max(model_values$Leverage, na.rm = TRUE))
    ylable_neg_0.5 <- 
      1.05 * -sqrt(0.5 * p * (1 - max(model_values$Leverage, na.rm = TRUE)) /
                     max(model_values$Leverage, na.rm = TRUE))
    ylable_pos_1 <-
      1.05 * sqrt(1 * p * (1 - max(model_values$Leverage, na.rm = TRUE)) /
                    max(model_values$Leverage, na.rm = TRUE))
    ylable_neg_1 <- 
      1.05 * -sqrt(1 * p * (1 - max(model_values$Leverage, na.rm = TRUE)) /
                     max(model_values$Leverage, na.rm = TRUE))
