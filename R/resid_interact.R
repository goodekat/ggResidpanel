...
resid_interact <- function(
  ...
  if ("boxplot" %in% plots | "SAS" %in% plots | "all" %in% plots) {
    ...
    if (title.opt == TRUE) {
      ...
      boxplot <- 
        ggplotly(boxplot, tooltip = c("Residual", "Data")) |>
  ...
  if ("cookd" %in% plots) {
    ...
    if (title.opt == TRUE) {
      title = helper_plotly_title(cookd)
      cookd <- 
        ggplotly(cookd, tooltip = c("y", "Data")) |>
        layout(annotations = title, title = FALSE)
    } else {
  ...
  if ("hist" %in% plots |
    ...
    if (title.opt == TRUE) {
      ...
      hist <- 
        ggplotly(hist, tooltip = c("Data", "density", "fill")) |>
        layout(annotations = title, title = FALSE)
    } else {
  ...
  if ("index" %in% plots | "default" %in% plots | "all" %in% plots) {
    ...
    if (title.opt == TRUE) {
      ...
      index <- 
        ggplotly(index, tooltip = c("Observation", "Residual", "Data")) |>
        layout(annotations = title, title = FALSE)
    } else {
    ...
    if (title.opt == TRUE) {
      ...
      lev <- 
        suppressWarnings(style(
          ggplotly(
            lev, 
            tooltip = c("Leverage", "Std_Res", "CooksD", "Data")
          ),
          hoverinfo = "skip", 
          traces = c(6))
        ) |>
    ...
    if (title.opt == TRUE) {
      ...
      lev <- 
        suppressWarnings(
          ggplotly(
            lev, 
            tooltip = c("Leverage", "Std_Res", "CooksD")
          ) |>
            layout(annotations = title, title = FALSE)
        )
    } else {
      lev <- 
        suppressWarnings(
          ggplotly(lev, tooltip = c("Leverage", "Std_Res", "CooksD"))
        )
    }
  } else {
    lev = NULL
      ...
      if (class(model)[1] == "lm") {
        ls <- 
          suppressWarnings(ggplotly(
            ls + labs(x = "Predicted Values", y = "sqrt(|Standardized Residuals|)"),
        ...
        ls <- 
          suppressWarnings(ggplotly(
            ls + labs(x = "Predicted Values", y = "sqrt(|Standardized Deviance Residuals|)"),
            tooltip = c("Prediction", "Sqrt_Std_Res", "Data")
          )) |>
          layout(annotations = title, title = FALSE)
        ...
        ls <- 
          suppressWarnings(ggplotly(
            ls + labs(x = "Predicted Values", y = "sqrt(|Standardized Pearson Residuals|)"),
            tooltip = c("Prediction", "Sqrt_Std_Res", "Data")
          )) |>
          layout(annotations = title, title = FALSE)
      }
  ...
  if ("qq" %in% plots | 
    ...
    if (title.opt == TRUE) {
      ...
      qq <- 
        ggplotly(qq, tooltip = c("Data", "Residual", "Theoretical")) |>
        layout(annotations = title, title = FALSE)
    } else {
  ...
  if ("resid" %in% plots | 
    ...
    if (title.opt == TRUE) {
      ...
      resid <- 
        ggplotly(resid, tooltip = c("Prediction", "Residual", "Data")) |>
        layout(annotations = title, title = FALSE)
    } else {
