...
The static panel of plots do not allow one to determine which observtions are associated with a point. The function `resid_interact` allows users to interact with the plots by using functionality from plotly. The code below reproduces the plot above with interactivity. When the cursor is hovered over a point, a tooltip appears with the x and y location of the point, the observed values associated with the point from variables included in the model, and the observation number. This function is meant to help users identify outliers and points of interest in the plot.
...
```{r}
resid_panel(penguin_model_log2, plots = "yvp")
```
...
```{r}
resid_auxpanel(penguin_tree_resid, penguin_tree_pred, plots = c("resid", "index"))
```
