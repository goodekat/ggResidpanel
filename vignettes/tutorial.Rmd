...
knitr::opts_chunk$set(echo = TRUE, fig.align = 'center', message = FALSE, cache = TRUE)
```
...
The functions in this vignette will be demonstrated by using the `penguins` data included the package. The dataset contains information on the heart rate (beats per minute), depth (meters), and duration (minutes) of dives by 9 emperor penguins. Multiple observations are recorded for each penguin. A total of 125 observations are in the dataset. The first six rows of the data are shown below.
...
The number of observations per bird are shown below.

```{r}
# Counts the number of observations per penguin
penguins %>% count(bird) %>% knitr::kable()
```

A linear mixed model is fit below to determine if there is a linear relationship between the heart rate of a penguin and the depth and duration of the dive. For now, the variables of depth and duration are included in the model as main effects only. A random effect is included to account for the dependence of observations within a penguin. This model will be used for examples throughout this vignette.

```{r}
# Fits a linear model with a response variable of heart rate and predictor
# variables of depth and duration
...
3. Specify the name of a [prespecified panel](#prespecified) of plots included in the package
...
resid_panel(penguin_model, plots = "boxplot")
```
...
resid_panel(penguin_model, plots = "cookd")
```
...
resid_panel(penguin_model, plots = "hist")

# Creates a histogram with 20 bins
resid_panel(penguin_model, plots = "hist", bins = 20)
```
...
resid_panel(penguin_model, plots = "index")

# Creates an index plot with a smoother added
resid_panel(penguin_model, plots = "index", smoother = TRUE)
```
...
resid_panel(penguin_model, plots = "lev")

# Creates a residual-leverage plot with a smoother added
resid_panel(penguin_model, plots = "lev", smoother = TRUE)
```
...
resid_panel(penguin_model, plots = "ls")

# Creates a location-scale plot of the residuals with a 
resid_panel(penguin_model, plots = "ls", smoother = TRUE)
```
...
resid_panel(penguin_model, plots = "qq")

# Creates a qq-plot with the 1-1 line removed
resid_panel(penguin_model, plots = "qq", qqline = FALSE)

# Creates a qq-plot with the confidence bands added
resid_panel(penguin_model, plots = "qq", qqbands = TRUE)
```
...
resid_panel(penguin_model, plots = "resid")

# Creates a plot with the smoother added
resid_panel(penguin_model, plots = "resid", smoother = TRUE)
```
...
resid_panel(penguin_model, plots = c("resid", "qq"))

# Creates a panel from a vector of four plot names
resid_panel(penguin_model, plots = c("hist", "ls", "cookd", "lev"))

# An example to show what happens if a plot name is included multiple times in the vector
resid_panel(penguin_model, plots = c("resid", "resid"))
```
...
resid_panel(penguin_model, plots = "default")
```
...
resid_panel(penguin_model, plots = "R", smoother = TRUE)
...
plot(penguin_model)    # Creates the base R diagnostic plots
...
resid_panel(penguin_model, type = "standardized")
```
...
resid_panel(penguin_model, axis.text.size = 14, title.text.size = 16)
```
...
resid_panel(penguin_model, plots = c("lev", "yvp", "ls", "cookd"))

# Creates the same panel of plots shown above with a scale of 0.9 
resid_panel(penguin_model,  plots = c("lev", "yvp", "ls", "cookd"), scale = 0.9)
```
...
resid_panel(penguin_model, theme = "classic")

# Creates the default panel with the grey theme
resid_panel(penguin_model, theme = "grey")
```
...
resid_panel(penguin_model, title.opt = FALSE)
```
...
3. Specify the name of a [prespecified panel](#interact_pre) of plots included in the package
...
resid_interact(penguin_model, plots = "boxplot")
```
...
resid_interact(penguin_model, plots = "cookd")
```
...
resid_interact(penguin_model, plots = "hist")

# Creates an interactive histogram with 20 bins
resid_interact(penguin_model, plots = "hist", bins = 20)
```
...
resid_interact(penguin_model, plots = "index")

# Creates an interactive index plot with a smoother
resid_interact(penguin_model, plots = "index", smoother = TRUE)
```
...
resid_interact(penguin_model, plots = "lev")

# Creates an interactive residual-leverage plot with a smoother
resid_interact(penguin_model, plots = "lev", smoother = TRUE)
```
...
resid_interact(penguin_model, plots = "ls")

# Creates an interactive location-scale plot with a smoother
resid_interact(penguin_model, plots = "ls", smoother = TRUE)
```
...
resid_interact(penguin_model, plots = "qq")

# Creates a normal quantile plot with the 1-1 line removed
resid_interact(penguin_model, plots = "qq", qqline = FALSE)
```
...
resid_interact(penguin_model, plots = "resid")

# Creates the residual plot with a smoother
resid_interact(penguin_model, plots = "resid", smoother = TRUE)
```
...
resid_interact(penguin_model, plots = "yvp")
```
...
# Creates a panel from a vector of four plot names
resid_interact(penguin_model, plots = c("hist", "ls", "cookd", "lev"))
```
...
resid_interact(penguin_model, plots = "all")
```
...
resid_interact(penguin_model, plots = "default")
```
...
resid_interact(penguin_model, plots = "R", smoother = TRUE)
```
...
resid_interact(penguin_model, plots = "SAS")
```
...
The first argument in `resid_xpanel` is the `model` option. The most basic use of `resid_xpanel` is to input a model into the function as shown below. This creates a panel of scatterplots of the residuals from the model versus the predictor variables in the model. Two predictor variables were included in the `penguin_model` (height and girth). As a result, the code below produces a panel with two plots.

```{r}
# Creates a panel of residuals versus the predictor variables in the penguin_model
...
resid_xpanel(penguin_model, yvar = "residual")
...
The first argument in `resid_compare` is the `models` option, which requires a list of models. The most basic use of `resid_compare` is to only input a list of models into the function. The diagnostic plots from from the `penguin_model` suggest possible issues with linearity and homogeneity. The code below fits two new models that attempt to deal with these issues. The three models of `penguin_model`, `penguin_model_squared`, and `penguin_model_log` are input into `resid_compare` as a list. This creates a 4x3 panel of plots. The columns represent the models, and the rows are different plot types. Each plot type is created for all three models. By default, the four plots in the default `resid_panel` appear. `resid_compare` includes options to change the plots and adjust the appearance of the figure.

```{r fig.height = 8, fig.width = 12}
# Fits a model with the log transformed volume variable as the response
penguin_model_log <- lm(log(Volume) ~ Height + Girth, data = trees)

# Fits a model with an added squared term of girth
penguin_model_squared <- lm(Volume ~ Height + Girth + I(Girth^2), data = trees)

# Creates a panel with the four default diagnostic plots for all three models
resid_compare(models = list(penguin_model, penguin_model_log, penguin_model_squared))
```
...
resid_compare(models = list(penguin_model, penguin_model_log), 
