---
title: Creating Panels of Diagnostic Plots with <span style="color:blue">ggResidpanel</span>
...
The package contains the following five functions. Note that `resid_panel`, `resid_interact`, `resid_xpanel`, and `resid_compare` are designed to work with models of type "lm", "glm", "lmer", "glmer", and "lmerTest".
...
A section is included in this vignette for each of the functions with details and examples showing how to use the functions.
...
This function and the next three functions will be demonstrated by fitting a model to the dataset `trees` included in base R. The dataset contains information on the volume, girth, and height of 31 black cherry trees. The linear model below is fit to determine if their is a linear relationship between the volume of the tree given its height and girth.
